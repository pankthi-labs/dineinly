...
| **Menu Item** | restaurant_id, category_id, name, description, sort, price, prep_time (enum, bucketed ranges), serving_size (enum), diet (enum), availability, labels (text[] of Menu Label names), offers_spice/offers_salt/offers_ice (booleans) | Sellable catalog item. `sort` is display order within its category — same pattern as Menu Category's own `sort`, new dishes append to the end. `labels` stores the chosen names directly, not label ids — no join table — but every value is validated server-side against that restaurant's Menu Label set. `offers_spice`/`offers_salt`/`offers_ice` only gate whether the guest sees that preference at order time; they never affect price and carry no value themselves — the guest's actual choice (mild/regular/extra spicy, etc.) lives on Cart Item / Order Item, not here. |
