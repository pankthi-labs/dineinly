...
| **Bill** | restaurant_id, session_id (1:1), bill_number, status (open/requested/settled), service_charge_rate (snapshot), subtotal, tax_amount, service_charge_amount, total (all nullable until settle), settled_at, settled_by | Financial record for the session. Own table: a bill has its own lifecycle (open → requested → settled) distinct from the session's, and is the highest-value table to keep clean for future settlement/reprint features. Amounts are **derived on read** for presentation (open/requested) and **frozen** only at settle — see lifecycle. Dineinly never processes payment, so no settlement-method column — only *that* it settled. `bill_number` is the human-facing identifier restaurant staff and guests reference (unlike `id`, which stays internal) — a random 9-digit integer, unique per restaurant, drawn the first time a session's bill is requested; it doesn't reveal order/volume to guests and never repeats within a restaurant. |
...
- **Request bill:** create/mark Bill `requested`, snapshot current tax/service rates onto it. Guests can request the bill anytime; displayed totals are derived on read from Order Items until settle. On the session's first request, also draws a random `bill_number` — later requests for the same session are pure status/rate re-snapshots and never redraw it.
