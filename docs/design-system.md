...
- **Exception — public home page motion:** `apps/web/app/home-content.tsx` (rendered by `apps/web/app/page.tsx`) is the only file in the app permitted to import the `motion` package (MIT license, the framer-motion successor) for entrance and scroll-triggered animation. It's a marketing page, not product UI, and needs latitude the rest of the app's §05 CSS-only motion doesn't give it. No other route may add `motion` as a dependency or import it — product UI stays on the CSS keyframe/transition tokens in §05.
- **Exception — home page loop card (`.glass` + `--ease-emphasized`):** the four hover-reveal cards in `DineinlyLoop` (`apps/web/app/home-content.tsx`) use `.glass` for a floating info card that is not a Bottom Sheet, Side Dock, or Modal, and reuse `--ease-emphasized` for the card reveal and node entrance, which §05 otherwise reserves for Bottom Sheet/Side Dock open. One-time, page-scoped sign-off for this single hero interaction — the page's most-seen moment, not a repeatable pattern. Do not reuse `.glass` on a hover/tooltip surface or `--ease-emphasized` outside Bottom Sheet/Side Dock anywhere else without separate sign-off.
- **Exception — home page gold accent (`apps/web/app/home-content.tsx`, this file only, never elsewhere):** `--color-accent-primary` is used outside §06/§11's whitelist in exactly six spots — the "Dineinly Experiences" `<h2>` (`text-accent`), the `TRACKS` package icons and `PRINCIPLES` icons (`text-accent`, a standalone icon accent not tied to an active/selected state — §11 forbids this), the `TRACKS` card's `hover:border-accent/40` (a decorative container border — §06 forbids this by name), the `DineinlyLoop` node's resting `border-accent/40` + `group-hover:border-accent` icon-circle ring, and the ripple's `border-accent/60`. All six are thin borders or small glyphs, never a fill or color block; the 12% screen-coverage cap and "never body text" rule still hold. Page-scoped sign-off only — no other route may color an icon gold outside an active/selected state, or put a gold border on anything, without separate sign-off.
- **Exception — home page heading measure (`apps/web/app/home-content.tsx`, this file only, never elsewhere):** the hero `<h1>` (`max-w-[16ch]`), the "Dineinly Experiences" lead paragraph (`max-w-[22ch]`), the `DineinlyLoop` lead paragraph (`max-w-[36ch]`), and the "Simple for guests" `<h2>` (`max-w-[20ch]`) constrain wrap width with a `ch`-based `max-w` alongside `text-balance`, instead of the `.prose` utility's fixed 68ch (§02, which governs body copy, not short display headings/leads). Page-scoped, these four elements only — body copy on this page still uses `.prose`.
- **Exception — home page off-grid geometry (`apps/web/app/home-content.tsx`, this file only, never elsewhere):** three arbitrary values in `DineinlyLoop` fall between `--space-*` steps and have no compliant token: the node tooltip card's `w-[14rem]` (224px, a readable card width, not a spacing value), the connecting line's `top/right/left-[1.375rem]` (22px — exactly half the 44px icon circle below, positioning the line at the circle's vertical center), and the ripple ring's `inset-[-0.4rem]` (the ring's outward offset from that same circle). All three are geometry derived from the 44px icon-circle size (itself §11's own sanctioned tap-target minimum, not a deviation), not independently-chosen arbitrary style. This project's Tailwind spacing scale has no dynamic multiplier (`--spacing: initial` in `globals.css`, only the named `--space-*` steps are defined), so there is no non-arbitrary way to express "half of 44px" or "224px card" — bracket values are the only mechanism available, not a shortcut around one. Page-scoped; do not introduce new off-grid bracket values elsewhere without checking the `--space-*` table first.
