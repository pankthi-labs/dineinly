...
5. **Attribution stays separate.** Once a device is paired, individual staff identify via PIN (`Staff.pin_hash`) as already described above — attribution/audit/UI only, no DB auth power, no relationship to pairing. A successful (re)pairing always clears any PIN session left on the device (`/floor` re-prompts for a PIN), so a fresh redemption never inherits whoever was last acting on it. The device's own header "Profile" action shows the PIN-unlocked staff member's name and role read-only — it never edits the shared station identity; self-service name/PIN changes stay individual, only reachable by signing in at `/sign-in`.
