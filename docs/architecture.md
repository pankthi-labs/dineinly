...
Table procedures live in `apps/web/server/routers/tables.ts`, gated by `authedProcedure` (any signed-in Dineinly Admin or active staff member of the restaurant) — not a stricter Owner/Manager-only check. Product.md's RBAC matrix lists "Manage Tables & QR Codes" as Owner/Manager/Admin only, but that per-role split isn't modeled anywhere yet (same deferral as "Manage Menu" — see `staff_all_menu_categories` in the RLS migration and "Feature-level staff permissions" in `Tbd.md`); Table Matrix follows the same any-active-staff reach as Menu Desk until that split ships.
