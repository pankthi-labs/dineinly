...
`RestaurantNavHeader`'s file also exports `DirectoryLink`, rendered inline in the header's own action row (next to `AdminHeaderActions`) — an icon-only `ArrowLeftFromLine` link back to `/admin/restaurants`, labelled for screen readers via `aria-label`/`title` rather than visible text. `DirectoryLink` renders `null` for non-admin viewers: a restaurant's own staff has no directory to leave to.
