...
describe("computeBill", () => {
	it("matches the reference bill: merged lines, CGST/SGST split, service charge", () => {
		...
		expect(result.lines).toEqual([
			{ name: "Truffle Risotto", unitPrice: 1400, quantity: 3, amount: 4200 },
			{ name: "Pinot Noir Glass", unitPrice: 800, quantity: 1, amount: 800 },
			{ name: "Burrata Salad", unitPrice: 650, quantity: 1, amount: 650 },
		]);
		expect(result.subtotal).toBe(5650);
		expect(result.taxSlabs).toEqual([
			{ ratePercent: 2.5, cgst: 141.25, sgst: 141.25 },
		]);
		expect(result.serviceCharge).toBe(282.5);
		expect(result.total).toBe(6215);
	});
	...
	it("omits a zero-rate slab and handles no service charge", () => {
		const result = computeBill(
